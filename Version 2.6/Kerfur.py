...
petting = False
petting_stage = 0 # 0 not being pet, 1 petting face #1, 2 petting face #2
petting_duration = 500
petting_start = 0
...
def reload_config():
    global config, color, Water_Alert, Silence, DebugMode, Name, Meow_Chance, Always_Top, window
    ...
    DebugMode = config.get("Debug", "False").lower() == "true"
    Always_Top = config.get("Always_Top", "False").lower() == "true"
    apply_tint()
    Name = config.get("Name")
    pygame.display.set_caption(f"{Name} | Desktop Kerfur")
    window = pwc.getWindowsWithTitle(f"{Name} | Desktop Kerfur")
...
def main():
    ...
    global last_config_time, DebugMode, color, Always_Top
    global petting, petting_duration, petting_start, petting_stage
    global window
    ...
    while run:
        ...
        if petting:
            elapsed = current_time - petting_start
            if petting_stage == 1 and elapsed >= 150:
                petting_stage = 2
            elif petting_stage == 2 and elapsed >= 150 + meow_duration:
                petting = False
        ...
        if current_time - petting_start >= petting_duration:
            petting = False
        ...
        if window and Always_Top:
            window[0].alwaysOnTop(True)
        else:
            window[0].alwaysOnTop(False)
